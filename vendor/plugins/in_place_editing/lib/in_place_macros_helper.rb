module InPlaceMacrosHelper
  ...
  def in_place_editor_field(object, method, tag_options = {}, in_place_editor_options = {})
    ...
    controller    = object.to_s.pluralize
    ...
    in_place_editor_options[:url] = in_place_editor_options[:url] || url_for({ :controller => controller, :action => "set_#{object}_#{method}", :id => instance_tag.object.id })
    
    if !( h( instance_tag.value instance_tag.object ) ).blank?
      text_value = h( instance_tag.value instance_tag.object )
    else
      text_value = 'no especificado'
    end
    
    tag = content_tag(tag_options.delete(:tag), text_value, tag_options)
